...
namespace debugOverlay
{
  ...
  void debugOverlayInfo::destroyOverlayInfo()
  {
    ...
    if(m_overlayContainerShadow)
    {
      _debug.m_overlay->remove2D(m_overlayContainerShadow);
    }
    ...
    if(m_overlayContainerShadow)
    {
      manager.destroyOverlayElement(m_overlayContainerShadow);
    }
  }
  //------------------------------------------------------------------------------//
  ...
  void debugOverlay::out(char* fmt, ...)
  {
    ...
    try
    {
      ...
      if(count == 0)    // строка изменилась
      {
        ...
        for(uint16 pos = 1; pos < __DEBUG_COUNT_LINE; pos++)
        {
          m_countCharInLine[pos - 1] = m_countCharInLine[pos];
        }
...
#define debugOverlay_add(_type)   class debugOverlayInfo_##_type : public debugOverlayInfo  {   public:     bool update()     {       try {         if ( ::IsBadReadPtr( (const void*)m_value, sizeof(_type) ) ) return false;      } catch (...) { return false;}      String strOut = m_strName + " = " + StringConverter::toString( *((_type*)m_value) );      if (m_overlayContainer->getCaption() != strOut) {         m_overlayContainer->setCaption(strOut);         if (m_overlayContainerShadow) m_overlayContainerShadow->setCaption(strOut);       }       return true;    };  };  void debugOverlay::add(const String & name, uint16 x, uint16 y, _type & value)  {     check();    remove(name);     debugOverlayInfo * info = new debugOverlayInfo_##_type();     info->createOverlayInfo(name, x, y, (void*)&value);     mOverlayInfo.push_back(info);   }
  //------------------------------------------------------------------------------//
  ...
  void debugOverlay::check()
  {
    ...
    while(fonts.hasMoreElements())
    {
      ...
      if(name == __DEBUG_FONT_DEFAULT)
      {
        m_strFontName = __DEBUG_FONT_DEFAULT;
      }
    }
