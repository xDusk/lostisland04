  ...
  *************************************************************************************/
  ...
  void CustomCopy::initCollection(void)
  {
    if(mCurrentCollection)
    {
      ...
      for(NxU32 i = 0; i < mCurrentCollection->mConvexMeshes.size(); i++)
      {
        mCurrentCollection->mConvexMeshes[i]->mInstance = 0;
      }

      for(NxU32 i = 0; i < mCurrentCollection->mTriangleMeshes.size(); i++)
      {
        mCurrentCollection->mTriangleMeshes[i]->mInstance = 0;
      }

      for(NxU32 i = 0; i < mCurrentCollection->mHeightFields.size(); i++)
      {
        mCurrentCollection->mHeightFields[i]->mInstance = 0;
      }

      for(NxU32 i = 0; i < mCurrentCollection->mSkeletons.size(); i++)
      {
        mCurrentCollection->mSkeletons[i]->mInstance = 0;
      }

      for(NxU32 i = 0; i < mCurrentCollection->mClothMeshes.size(); i++)
      {
        mCurrentCollection->mClothMeshes[i]->mInstance = 0;
      }
  ...
  void CustomCopy::initScene(void)
  {
    if(mCurrentScene)
    {
      for(NxU32 i = 0; i < mCurrentScene->mMaterials.size(); i++)
      {
        mCurrentScene->mMaterials[i]->mInstanceIndex = 0;
      }

      for(NxU32 i = 0; i < mCurrentScene->mActors.size(); i++)
      {
        mCurrentScene->mActors[i]->mInstance = 0;
      }

      for(NxU32 i = 0; i < mCurrentScene->mJoints.size(); i++)
      {
        mCurrentScene->mJoints[i]->mInstance = 0;
      }

      for(NxU32 i = 0; i < mCurrentScene->mEffectors.size(); i++)
      {
        mCurrentScene->mEffectors[i]->mInstance = 0;
      }

#if NX_SDK_VERSION_NUMBER >= 260

      for(NxU32 i = 0; i < mCurrentScene->mCompartments.size(); i++)
      {
        mCurrentScene->mCompartments[i]->mInstance = 0;
      }
