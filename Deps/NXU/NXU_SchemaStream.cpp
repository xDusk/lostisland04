...
#define DEBUG_WARNING 0
  ...
  class SchemaBlock
  {
    public:
      ...
      void store(NX_BOOL v, const char *name, bool attribute)
      {
        if(mFlushed)
        {
          assert(attribute == false);  // can't write new attributes after the header has been flushed!
        }
  ...
  void SchemaStream::setCurrentJoint(NxJointDesc *j)
  {
    mCurrentJoint = j;
  };
  void SchemaStream::setCurrentActor(NxActorDesc *a)
  {
    mCurrentActor = a;
  };
  void SchemaStream::setCurrentShape(NxShapeDesc *s)
  {
    mCurrentShape = s;
  };
  void SchemaStream::setCurrentFluid(NxFluidDesc *f)
  {
    mCurrentFluid = f;
  };
  void SchemaStream::setCurrentCloth(NxClothDesc *c)
  {
    mCurrentCloth = c;
  };
